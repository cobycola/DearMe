...
/**
 * 提交答案节点（用例：POST /api/sessions/{id}/answers）。
 * before：assertCanAnswer + 记录答案；deal：推理 + 选下题（无下题则 markAllAnswered）；after：落仓储。
 */
@Slf4j
@Component
public class SubmitAnswerNode extends AbstractSessionNodeService {

    private final TopicRegistryPort topicRegistry;
    private final InferenceEngine inferenceEngine;
    private final NextQuestionPolicy nextQuestionPolicy;

    public SubmitAnswerNode(SessionRepositoryPort sessionRepository,
    ...
    protected boolean before(SessionFlowContext context) {
        ...
        try {
            ...
            context.getSession().addAnswer(new Answer(context.getQuestionId(), context.getOptionIndex()));
        } catch (IllegalStateException e) {
            log.error("[SubmitAnswerNode] 提交答案非法状态: {}", e.getMessage());
    ...
    protected void deal(SessionFlowContext context) {
        ...
        if (context.getNextQuestion() == null) {
            SessionTransition.markAllAnswered(context.getSession());
        }
    }
